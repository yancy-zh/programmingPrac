...
public class CoinChange
{
	...
	{
		int max = amount + 1; // the biggest possible solution is amount, so plus 1 is bigger than all solutions.
		...
		for (int i = 1; i <= amount; i++)
		{
			...
			{// consider all history solutions, including dp[0]
				if (coins[j] <= i)
				{// loop in all elements in coins, get the dp memory and get the minimum,

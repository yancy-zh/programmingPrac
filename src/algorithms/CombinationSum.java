...
{
	/*
	 * 39. Combination Sum Given an array of distinct integers candidates and a target integer target, return a list of all unique combinations of candidates
	 ...
	 * Example 1:
	 * 
	 * Input: candidates = [2,3,6,7], target = 7 Output: [[2,2,3],[7]] Explanation: 2 and 3 are candidates, and 2 + 2 + 3 = 7. Note that 2 can be used multiple
	 * times. 7 is a candidate, and 7 = 7. These are the only two combinations. Example 2:
	 * 
	...
	public int[][] combinationSum(int[] candidates, int target)
	{
		...
		//TODO: change type of allCombis to Set
		return twoDListTotwoDint(allCombis);
	}
	...
	public int[][] twoDListTotwoDint(List<List<Integer>> listOfList)
	{
		int[][] dst = new int[listOfList.size()][];
		for (int i = 0; i < listOfList.size(); i++)
		{
			List<Integer> row = listOfList.get(i);
			dst[i] = new int[row.size()];
